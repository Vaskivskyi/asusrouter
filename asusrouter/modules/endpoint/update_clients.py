...
def read_legacy_as_json(content: str) -> dict[str, Any]:
    ...
    # Step 3 - Remove the `{}` in the beginning and end,
    # also clean trailing and leading spaces
    data = data[1:-1].strip()
    ...
    # Find all `, [a-zA-Z0-9_]+:` where [a-zA-Z0-9_]+
    # is the key and add `"` around the key
    data = re.sub(r", ?([a-zA-Z0-9_]+):", r',"\1":', data)

    # Step 6 - replace `customList` with `"customList"`
    data = data.replace("customList", '"customList"')

    # Step 7 - find `"wlList_2g": \[[.*]\]` as well as `"wlList_5g": \[[.*]\]`
    # and `"wlList_5g_2": \[[.*]\]` and replace them with `"wlList_2g": {[.*]}`
    # as well as `"wlList_5g": {[.*]}` and `"wlList_5g_2": {[.*]}`
    # Keep the content inside the brackets as is
    # Stop the regex search on the first occurence of the `]` so that we
    # don't match the last `]`
    data = re.sub(
        r'"wlList_2g": \[([^\]]*)\]', r'"wlList_2g": {\1}', data, count=1
    )
    data = re.sub(
        r'"wlList_5g": \[([^\]]*)\]', r'"wlList_5g": {\1}', data, count=1
    )
    data = re.sub(
        r'"wlList_5g_2": \[([^\]]*)\]', r'"wlList_5g_2": {\1}', data, count=1
    )
...
def read_legacy_networmap(
    output: dict[str, Any], networkmap: str | None
) -> None:
...
def read_legacy_nmpclient(
    output: dict[str, Any], nmp_client: str | None
) -> None:
...
def read_legacy_staticlist(
    output: dict[str, Any], static_list: str | None
) -> None:
...
def read_legacy_wlan_info(
    output: dict[str, Any], data: dict[str, Any]
) -> None:
    ...
    for wlan in LEGACY_WLAN:
        ...
        for client in wlan_info:
            ...
            if mac in output:
                ...
                output[mac]["wlConnectTime"] = wl_connect_time
                continue
            output[mac] = {
                "mac": mac,
                "curTx": cur_tx,
                "curRx": cur_rx,
                "wlConnectTime": wl_connect_time,
            }


def read_legacy_wlan_list(
    output: dict[str, Any], data: dict[str, Any]
) -> None:
