...
# Raw `macmode` value -> filter mode (SDN uses both numeric and text)
...
def _decode(raw: Any) -> str:
    """Decode the char-encoded rule-list separators to `<`/`>`."""
...
def _parse_sdn_rl(raw: Any) -> list[tuple[str, str, int, bool]]:
    ...
    for cols in _split_rules(_decode(raw)):
...
def _read_mac_list(raw: Any) -> list[MacAddress]:
    """Read a MAC filter list into MAC addresses."""

    return [
        mac
        for token in _MAC_RE.findall(_decode(raw))
...
class ARNetworkSource(ARDataSource):
    """AsusRouter network (SDN) data source."""
...
def _build_network(
    ...
    mode = _MAC_FILTER_MODES.get(str(_get("macmode")))
    if mode is not None:
        fields[ARNetworkField.MAC_FILTER_MODE] = mode
    mac_list = _read_mac_list(_get("maclist"))
    if mac_list:
